 ...
 * 套件版本号
 */
export const Version = "1.0.1912.1";
 ...
 */
export function isObject(obj) { return typeof val === "object"; }
 ...
 * @return {Boolean} 对象为空判断结果
 */
export function isNoneString(obj) {
    switch (typeof obj) {
        case "undefined":
            return true;
        case "object":
 ...
 */
export function getRndString(len) {
    len = len || 32;
 ...
 * UTF8编码操作器
 */
export var utf8 = new UTF8Encoding();

/**
 * 获取字符串MD5值
 * @param {String} s 字符串
 */
export function getMD5(s) {
    var objMD5 = {};
 ...
 */
export function upload(url, fsuccess) {
 ...
 * 获取一个表单管理器
 */
export function getForm(el) { return new FormManager(el); }
 ...
 * @param {*} str 
 */
export function jsonDeserialize(str) {
    try {
        var obj = eval('(' + str + ')');
        return obj;
    }
 ...
 * 创建一个加载器
 */
export function createLoader() { return new DynamicLoader(); }
 ...
 * @param {void} fn 
 */
export function loadCss(id, url, fn) {
 ...
 * @param {void} fn 
 */
export function loadJs(id, url, fn) {
    var loader = new DynamicLoader();
