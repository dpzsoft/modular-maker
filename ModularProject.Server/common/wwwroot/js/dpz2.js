...
class Network {
    ...
    ajaxStateChange(http_request, fsuccess, ferror) {
        if (http_request.readyState === 4) {
            ...
            if (http_request.status === 200) {
                ...
                if (dpz2.isNoneString(reqStr)) return;
                if (fsuccess)
                    fsuccess(reqStr);
            }
            else {
    ...
    upload(url, fileobj, fprocess, fsuccess, ferror) {
        ...
        var http_request = that.getHttpRequest();
        http_request.onreadystatechange = function () {
            that.ajaxStateChange(http_request, fsuccess, ferror);
        };
        ...
        input.addEventListener("change", function () {
            if (fnselect) {
                fnselect({
                    ...
                    upload: function (url) {
                        if (dpz2.isNoneString(url)) return;
                        //alert(url);
                        //console.log(url);
                        that.upload(url, input, fprocess, function (responseText) {
    ...
    */
    load(fn) {
        ...
        var loadStyles = function (fnStyles) {
            ...
            //加载样式
            that.loadCss(my.styles[idx].id, my.styles[idx].url, function () {
                idx++;
                loadStyles(fnStyles);
            });
        };
        ...
        var loadImages = function (fnImages) {
            ...
            //加载样式
            that.loadImage(my.images[idx], function () {
                idx++;
                loadImages(fnImages);
            });
        };
        ...
        var loadScripts = function (fnScripts) {
            ...
            //加载样式
            that.loadJs(my.scripts[idx].id, my.scripts[idx].url, function () {
                idx++;
